  ...
  return (
    <main className="min-h-screen bg-white text-gray-900">
      ...
      <section className="text-center py-24 bg-gradient-to-b from-blue-50 to-white">
        ...
        <button
          onClick={() => handleRedirect('https://app.rentfax.io/signup?partner=agency')}
          className="px-8 py-3 bg-[#1A2540] text-white rounded-lg font-semibold shadow-md hover:bg-[#2A3660] transition"
        >
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-5xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-6 text-[#1A2540]">Why Collection Agencies Choose RentFAX</h2>
      ...
      <section className="py-20 text-center">
        <h2 className="text-3xl font-bold text-[#1A2540] mb-8">Choose Your Plan</h2>
        <div className="flex flex-wrap justify-center gap-6">
          {[
            ...
            <div
              key={plan.name}
              className={`p-8 w-72 text-center border rounded-lg shadow-sm ${plan.featured ? "border-blue-500 border-2 shadow-lg" : "bg-white"}`}>
              <h3 className={`text-xl font-semibold ${plan.featured ? "text-blue-600" : ""}`}>
                {plan.name}
              </h3>
              ...
              <button
                onClick={() => handleRedirect('https://app.rentfax.io/signup?partner=agency&plan=' + plan.name.toLowerCase())}
                className={`mt-6 w-full px-4 py-2 text-white rounded-lg font-semibold transition ${plan.featured ? "bg-blue-600 hover:bg-blue-700" : "bg-[#1A2540] hover:bg-[#2A3660]"}`}>
                Get Started
              </button>
            </div>
          ))}
      ...
      <section className="py-20 bg-gray-50 text-center">
        <h2 className="text-3xl font-bold mb-8 text-[#1A2540]">Frequently Asked Questions</h2>
        <div className="max-w-3xl mx-auto text-left space-y-6 text-gray-700">
          <details className="bg-white p-4 rounded-md shadow-sm">
            <summary className="font-semibold cursor-pointer">How does RentFAX assign cases to agencies?</summary>
            <p className="mt-2 text-sm text-gray-600">
              Once a renter dispute is marked unresolved, our system automatically assigns it to the most relevant verified agency based on capacity, performance, and coverage area.
            </p>
          </details>
          <details className="bg-white p-4 rounded-md shadow-sm">
            <summary className="font-semibold cursor-pointer">Can we choose our own cases?</summary>
            <p className="mt-2 text-sm text-gray-600">
              You can set filters for preferred states, case types, or amounts. Otherwise, assignments are automatic to keep workloads balanced.
            </p>
          </details>
          <details className="bg-white p-4 rounded-md shadow-sm">
            <summary className="font-semibold cursor-pointer">When do we get paid?</summary>
            <p className="mt-2 text-sm text-gray-600">
              Agencies handle collections directly with renters or companies. RentFAX manages the compliance, assignment, and case tracking only.
            </p>
          </details>
        </div>
      </section>
      ...
      <section className="py-16 text-center">
        <h2 className="text-3xl font-bold text-[#1A2540] mb-4">Ready to grow your agency?</h2>
        <p className="text-gray-600 mb-6">Start receiving verified debt cases from across the country.</p>
        <button
          onClick={() => handleRedirect('https://app.rentfax.io/signup?partner=agency')}
          className="px-8 py-3 bg-[#1A2540] text-white rounded-lg font-semibold shadow-md hover:bg-[#2A3660] transition"
        >
