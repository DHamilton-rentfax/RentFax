...
import MatchFoundScreen from "../search/MatchFoundScreen";
...
export default function SearchRenterModalMaster({
  ...
  useEffect(() => {
    const saved = localStorage.getItem("searchModalState");
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem("searchModalState", JSON.stringify({ form: formState }));
  }, [formState]);
  ...
  async function defaultSearch(payload: SearchPayload): Promise<SearchResult> {
    ...
    if (!res.ok) throw new Error("Failed to search renter.");
    return res.json();
  }
  ...
  const handleIdentityCheck = () => {
    ...
    router.push(`/identity-check?renter=${searchResult.id}`);
  };
  ...
  const handleSendSelfVerify = async () => {
    ...
    try {
      await fetch("/api/self-verify/send", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ searchSessionId: searchResult?.id, renter: formState }),
      });
      toast({
        title: "Success",
  ...
  async function handleUnlockReport() {
    if (!searchResult?.id) return;
    try {
      const res = await fetch("/api/report/purchase", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ searchSessionId: searchResult?.id }),
      });
        ...
        >
          <div className="relative border-b border-gray-200 px-6 py-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-2xl font-semibold text-gray-900">Renter Search</h2>
                <p className="text-sm text-gray-500">AI-powered identity &amp; fraud screening.</p>
              </div>
              <div className="hidden flex-col items-end sm:flex">
                <p className="text-xs font-medium text-gray-600">
                  Confidence:{" "}
                  <span className={ confidence >= 80 ? "text-green-600" : confidence >= 50 ? "text-orange-500" : "text-red-600" }>
                    {confidenceLabel}
                  </span>{" "}
                ...
                <div className="mt-1 h-1.5 w-40 overflow-hidden rounded-full bg-gray-200">
                  <motion.div
                    ...
                    style={{ backgroundColor: confidence >= 80 ? "#16a34a" : confidence >= 50 ? "#f97316" : "#dc2626" }}
                  />
                </div>
              </div>
            </div>
          ...
          {activeStep === 1 && (
            <div className="border-t border-gray-200 px-6 py-4">
              <button
                ...
                className={`relative flex w-full items-center justify-center rounded-full px-4 py-2.5 text-sm font-semibold text-white transition ${ canSubmit && !isLoading ? "shadow-sm" : "cursor-not-allowed opacity-70" }`}
                ...
                {isLoading && (
                  <div className="absolute inset-0 overflow-hidden rounded-full">
                    ...
                    <div
                      ...
                      style={{ width: `${progress}%`, backgroundColor: "#000000", opacity: 0.15, transition: "width 180ms linear" }}
                    />
                  </div>
                )}
                <span className="relative flex items-center">
                  {isLoading ? (
                    <> <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Searching… {Math.round(progress)}% </>
                  ) : (
