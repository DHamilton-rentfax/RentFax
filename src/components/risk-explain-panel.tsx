...
import { Loader2, Sparkles, Shield, AlertTriangle, CheckCircle, Lightbulb } from 'lucide-react';
...
const ConfidenceBadge = ({ confidence }: { confidence: 'Low' | 'Medium' | 'High' }) => {
    ...
    const Icon = confidence === 'High' ? CheckCircle : confidence === 'Medium' ? Shield : AlertTriangle;
    return (
      ...
      {output && (
        <div className="space-y-4 pt-4 border-t">
            ...
            <div>
                ...
                <p className="whitespace-pre-wrap text-sm text-muted-foreground bg-secondary p-3 rounded-md border">{output.explanation}</p>
            </div>
            
             <div>
                <p className="text-sm font-medium mb-1 flex items-center gap-2"><Lightbulb className="text-primary"/> Recommendation</p>
                <p className="whitespace-pre-wrap text-sm text-muted-foreground bg-secondary p-3 rounded-md border">{output.recommendation}</p>
            </div>
        </div>
      )}
    </div>
