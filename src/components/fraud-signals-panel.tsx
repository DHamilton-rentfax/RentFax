      ...
      {signal.code === "duplicateIdentity" && (
        <>
          <p className="text-sm text-muted-foreground">
            The <Badge variant="secondary">{signal.identifier}</Badge>{" "}
            <Badge variant="destructive">{signal.value}</Badge> is shared by{" "}
            {signal.matches?.length || 0} other profile(s):
          </p>
          <ul className="list-disc pl-5 mt-2 text-sm space-y-1">
            {signal.matches?.map((match) => (
              <li key={match.id}>
                <Link
                  href={`/renters/${match.id}`}
                  target="_blank"
    ...
    try {
      ...
      if (res.signals.length === 0) {
        toast({
          ...
          description:
            "This renter has no overlapping identifiers or other risk signals in the network.",
        });
      }
    } catch (e: any) {
      toast({
