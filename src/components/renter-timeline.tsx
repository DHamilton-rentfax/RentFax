        ...
        {timeline.length === 0 ? (
          ...
          <div className="relative pl-6 after:absolute after:inset-y-0 after:w-px after:bg-border after:left-0">
            {timeline.map((item, index) => (
              <div
                key={index}
                className="relative grid grid-cols-[auto_1fr] gap-x-4 mb-6"
              >
                  ...
                  {item.type === "rental" ? (
                    <div className="text-sm text-muted-foreground">
                      <p>
                        ...
                        {(item.data as Rental).vehicle}
                      </p>
                      <p>
                        <strong>Duration:</strong>{" "}
                        {new Date(
                          (item.data as Rental).startAt,
                        ).toLocaleDateString()}{" "}
                        to{" "}
                        {new Date(
                          (item.data as Rental).endAt,
                        ).toLocaleDateString()}
                      </p>
                    </div>
                  ) : (
                    <div className="text-sm text-muted-foreground">
                      ...
                      <p>
                        ...
                        {(item.data as Incident).amount.toFixed(2)}
                      </p>
                    </div>
                  )}
                </div>
