...
export default function FraudScoreCard({
  renterId,
  userId,
}: {
  renterId: string;
  ...
  if (loading || !summary)
    return <div className="p-4">Loading fraud data...</div>;
  ...
  return (
    <div className="border rounded-lg p-4 shadow-sm space-y-3">
      ...
      <p className="text-sm text-muted-foreground">
        {summary.signals?.length || 0} signals flagged — status:{" "}
        {summary.status}
      </p>

      {summary.reviewedBy && (
        <p className="text-xs text-gray-400">
          Reviewed by: {summary.reviewedBy}
        </p>
      )}
