    ...
    <div className="fixed bottom-4 right-4 bg-white shadow-lg p-4 rounded-2xl w-80 border">
      <div className="h-64 overflow-y-auto space-y-4 text-sm p-2">
        {chat.map((c, i) => (
          ...
          >
            <p
              className={`p-2 rounded-lg max-w-[90%] ${c.role === "ai" ? "bg-muted text-foreground" : "bg-primary text-primary-foreground"}`}
            >
              <strong>{c.role === "ai" ? "AI: " : "You: "}</strong>
              {c.text}
            </p>
          </div>
        ))}
        {loading && (
          <div className="text-center text-muted-foreground">Thinking...</div>
        )}
      </div>
      <div className="flex mt-2">
        <Input
          ...
          disabled={loading}
        />
        <Button onClick={send} className="ml-2" disabled={loading}>
