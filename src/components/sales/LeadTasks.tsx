...
export function LeadTasks({ leadId }: { leadId: string }) {
  ...
  useEffect(() => {
    const q = query(collection(db, "tasks"), where("leadId", "==", leadId));
  ...
  const add = async () => {
    await createTask({ description: newTask, leadId });
    setNewTask("");
  };
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-2">
          {tasks.map((task) => (
            <div key={task.id} className="flex justify-between items-center border rounded-md p-2 text-sm">
