  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Use AI to draft a summary, suggest next steps, and generate a customer-facing note based on the incident details.
        </CardDescription>
      </CardHeader>
          ...
          <div className="mt-4 space-y-6 pt-4 border-t">
            {out.summary && (
              <div>
                ...
                <p className="text-sm text-muted-foreground bg-secondary p-3 rounded-md">{out.summary}</p>
              </div>
            )}
            {Array.isArray(out.checklist) && out.checklist.length > 0 && (
              <div>
                <h3 className="font-semibold text-lg mb-2 flex items-center gap-2"><CheckSquare /> Next Steps Checklist</h3>
                <ul className="list-disc pl-5 space-y-2 text-sm">
                  {out.checklist.map((c: string, i: number) => <li key={i}>{c}</li>)}
                </ul>
              </div>
            )}
            {out.customerNote && (
              <div>
                <h3 className="font-semibold text-lg mb-2 flex items-center gap-2"><MessageSquareText /> Suggested Customer Note</h3>
                 <p className="text-sm text-muted-foreground border-l-4 border-primary pl-4 py-2 bg-secondary">{out.customerNote}</p>
              </div>
            )}
          </div>
        )}
