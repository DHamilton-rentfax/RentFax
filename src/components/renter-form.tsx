...
import { upsertRenter } from '@/app/auth/actions';
...
const formSchema = z.object({
  ...
  dob: z.string().refine((val) => val && !isNaN(Date.parse(val)), {message: "Invalid date format"}),
...
export default function RenterForm({ renter, onSave }: RenterFormProps) {
  ...
  const onSubmit = async (values: FormData) => {
    ...
    try {
        ...
        toast({ title: 'Renter Saved', description: `${values.name} has been saved.` });
        onSave();
    } catch(e: any) {
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Notes</FormLabel>
                <FormControl>
                  <Textarea placeholder="Internal notes about this renter..." {...field} />
                </FormControl>
              <FormMessage />
            </FormItem>
          )}
