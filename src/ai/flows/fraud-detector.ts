...
export type DetectFraudSignalsInput = z.infer<
  typeof DetectFraudSignalsInputSchema
>;
...
export type DetectFraudSignalsOutput = z.infer<
  typeof DetectFraudSignalsOutputSchema
>;

export async function detectFraudSignals(
  input: DetectFraudSignalsInput,
  auth?: FlowAuth,
...
const detectFraudSignalsFlow = ai.defineFlow(
  ...
  async ({ renterId }, { auth }) => {
    ...
    const identityFields = ["email", "phone", "licenseNumber"].filter(
      (f) => !!renter[f],
    );
