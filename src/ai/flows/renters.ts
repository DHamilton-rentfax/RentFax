...
export type UndoRenterImportOutput = z.infer<typeof UndoRenterImportOutputSchema>;

export async function undoRenterImport(input: UndoRenterImportInput, auth?: FlowAuth): Promise<UndoRenterImportOutput> {
...
const undoRenterImportFlow = ai.defineFlow({
    ...
    if (data.companyId !== companyId) throw new Error('Permission denied. This batch belongs to another company.');
