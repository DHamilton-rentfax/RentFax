...
const createMockRental = (renterId: string): Rental => {
    ...
    return {
        ...
        status: faker.helpers.arrayElement(['active', 'completed', 'cancelled', 'overdue']),
    }
}
...
const createMockIncident = (rentalId: string): Incident => {
    return {
        id: faker.string.uuid(),
        rentalId,
        type: faker.helpers.arrayElement(['Damage', 'Late Return', 'Non-Payment', 'Smoking', 'Other']),
...
export const generateMockData = (type: 'renters' | 'rentals' | 'incidents' | 'all', count: number = 10) => {
  if (type === 'renters') {
  ...
  if (type === 'all') {
    ...
    const rentals = renters.flatMap(renter => Array.from({length: faker.number.int({min: 1, max: 3})}, () => createMockRental(renter.id)));
    const incidents = rentals.flatMap(rental => Array.from({length: faker.number.int({min: 0, max: 2})}, () => createMockIncident(rental.id)));
