...
export async function requireRole(roles: string[], sessionCookie?: string | null) {
  ...
  try {
    ...
    if (!user || !roles.includes(user.role)) {
      console.warn(`Blocked user ${decoded.email} from route requiring one of roles: ${roles.join(', ')}`);
      redirect("/dashboard");
    }
