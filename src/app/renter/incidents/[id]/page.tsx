...
export default function IncidentDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    const fetchIncident = async () => {
      ...
      if (incidentSnap.exists()) {
        setIncident({ id: incidentSnap.id, ...incidentSnap.data() } as Incident);
      } else {
  ...
  const isDisputable = incident.status === 'Open' || incident.status === 'pending';
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      ...
      <div className="bg-white shadow-md rounded p-6 mb-6">
        ...
        <p><strong>Status:</strong> <span className={`font-semibold ${incident.status === 'Open' || incident.status === 'pending' ? 'text-red-500' : 'text-green-500'}`}>{incident.status}</span></p>
        <p><strong>Date Reported:</strong> {new Date(incident.createdAt?.toDate?.() ?? incident.createdAt).toLocaleDateString()}</p>
        ...
        {incident.tags && incident.tags.length > 0 && (
            <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                    {incident.tags.map(tag => <span key={tag} className="badge">{tag}</span>)}
                </div>
            </div>
        )}
          ...
          <div>
            ...
            <ul>
              {incident.evidence.map((url, index) => (
                <li key={index}>
                  <a href={url} target="_blank" rel="noopener noreferrer">View Evidence {index + 1}</a>
                </li>
              ))}
            </ul>
          </div>
      ...
      {isDisputable && (
        <div className="bg-white shadow-md rounded p-6">
          ...
          <form onSubmit={handleDisputeSubmit} className="space-y-4">
            <textarea value={disputeMessage} onChange={(e) => setDisputeMessage(e.target.value)} placeholder="Explain why this report is incorrect..." required />
            <div>
                <label>Upload Evidence (optional)</label>
                <input type="file" multiple onChange={(e) => setDisputeFiles(Array.from(e.target.files ?? []))} />
            </div>
            <Button type="submit" disabled={submitting}>{submitting ? 'Submitting...' : 'Submit Dispute'}</Button>
          </form>
        </div>
      )}
      ...
      {!isDisputable && (
         <div role="alert">
            <p>This incident is currently <strong>{incident.status}</strong> and cannot be disputed at this time.</p>
        </div>
      )}
    </div>
  );
