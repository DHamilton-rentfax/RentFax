...
export default function ReadinessPage() {
  ...
  useEffect(() => {
    if (claims?.companyId) {
      ...
      // claims loaded but no companyId
      setLoading(false);
    }
  }, [claims]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        mailQueued = -1; /* ignore if access denied */
      }
  ...
  const checks: Check[] = useMemo(() => {
    ...
    return [
      {
        id: "plan",
        label: `Company plan active`,
        pass: company ? status === "active" : null,
        hint: `Status: ${status}, Plan: ${plan}`,
        link: "/settings/billing",
      },
      {
        id: "functions",
        label: `Cloud Functions reachable`,
        pass: !!health?.ok,
        hint: `Version ${health?.version || "..."} @ ${health?.timestamp || ""}`,
      },
      {
        id: "env",
        label: "Critical env vars set (Stripe, Tasks, Email)",
        pass: Object.keys(envFlags).length
          ? Object.values(envFlags).every(Boolean)
          : null,
        hint: `Found: ${Object.entries(envFlags).filter(([, v]) => v).length} of ${Object.keys(envFlags).length}`,
      },
      {
        id: "data-min",
        label: "Seeded data present (renters/rentals/etc)",
        pass:
          counts.renters > 0 &&
          counts.rentals > 0 &&
          counts.incidents > 0 &&
          counts.disputes > 0,
        hint: `renters=${counts.renters}, rentals=${counts.rentals}, incidents=${counts.incidents}`,
        link: "/admin/seed",
      },
      {
        id: "invites",
        label: "Invite flow configured",
        pass: counts.invites >= 0,
        hint: `Pending invites: ${counts.invites}`,
        link: "/settings/team",
      },
      {
        id: "risk",
        label: "Risk score engine updating renters",
        pass: scoreOK,
        hint: scoreOK
          ? "At least one renter has a score"
          : "No score found yet; create an incident to trigger",
      },
      {
        id: "indexes",
        label: "Composite indexes in place (probe query OK)",
        pass: indexOK,
        hint: "If false, run firebase deploy --only firestore:indexes",
      },
      {
        id: "mail",
        label: "Email pipeline installed (e.g., SendGrid)",
        pass: counts.mailQueued >= 0,
        hint: `Mail queue size: ${counts.mailQueued === -1 ? "Unknown" : counts.mailQueued}`,
      },
    ];
      ...
      <div className="max-w-3xl mx-auto p-4 md:p-10 space-y-6">
        ...
        >
          ...
          {!passAll && (
            ...
            >
              Fix the failed items above (marked with an "!") and refresh the
              page.
            </CardContent>
          )}
        </Card>
