...
export default function FraudDashboard() {
  ...
  const signalTypes = [
    "ALL",
    ...Array.from(new Set(allSignals.map((s) => s.type))),
  ];
          ...
          signalTypes.map((type) => (
            <TabsContent key={type} value={type}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                {filtered
                  .filter((s) => filter === "ALL" || s.type === type)
                  .map((s, i) => (
                    <Card key={i} className="border-l-4 border-red-500">
                      <CardContent className="p-4">
                        <div className="flex justify-between items-start">
                          <div>
                            <Link
                              href={`/admin/fraud/${s.renterId}`}
                              className="hover:underline"
                            >
                              <h2 className="font-semibold text-md">
                                {s.renterName || "Unknown Renter"}
                              </h2>
                            </Link>
              ...
              {filtered.filter((s) => filter === "ALL" || s.type === type)
                .length === 0 && (
                <div className="text-center p-8 text-gray-500">
                  No signals of this type found.
                </div>
              )}
            </TabsContent>
          ))}
