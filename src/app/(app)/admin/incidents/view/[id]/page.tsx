...
export default function AdminIncidentDetailPage({
  ...
  useEffect(() => {
    ...
    const fetchIncidentAndDispute = async () => {
      ...
      if (incidentSnap.exists()) {
        ...
        if (
          incidentData.status === "under_review" ||
          incidentData.status === "resolved" ||
          incidentData.status === "dismissed"
        ) {
          ...
          if (!disputeSnaps.empty) {
            const disputeData = disputeSnaps.docs[0].data() as Omit<
              Dispute,
              "id"
            >;
  ...
  if (!incident)
    return (
      <div className="max-w-4xl mx-auto py-8 px-4">Incident not found.</div>
    );
    ...
    <div className="max-w-4xl mx-auto py-8 px-4 space-y-8">
      ...
      {dispute && (
        <div className="bg-yellow-50 shadow-lg rounded-lg p-6">
          ...
          <p>
            <strong>Dispute Submitted On:</strong>{" "}
            {new Date(dispute.createdAt?.toDate()).toLocaleString()}
          </p>
          <p>
