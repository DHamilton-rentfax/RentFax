...
export default function AdminDisputePage() {
  ...
  if (!dispute) return <p className="p-8 text-center text-destructive">Dispute not found.</p>
    ...
    <div className="p-4 md:p-8">
      ...
      <Card className="max-w-3xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            ID: {dispute.id} | Submitted: {dispute.createdAt ? formatDistanceToNow(dispute.createdAt.toDate(), { addSuffix: true }) : 'N/A'}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <h3 className="font-semibold">Renter's Statement</h3>
            <p className="text-muted-foreground bg-secondary p-3 rounded-md">{dispute.description}</p>
          </div>
        ...
        <CardFooter className="flex items-center gap-2 pt-6 border-t">
          <span className="text-sm font-medium">Set Status:</span>
            ...
            <Button
                ...
                size="sm"
            >
                {updating && status === s && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
