 ...
 */
async function findConnectedRenters(renterId: string, renterData: any): Promise<string[]> {
    ...
    if (phone) {
        const phoneSnap = await adminDB.collection('renters').where('phone', '==', phone).get();
    ...
    if (address) {
        const addressSnap = await adminDB.collection('renters').where('address', '==', address).get();
 ...
 */
async function analyzeClusterForFraud(targetRenterId: string, connectedRenterIds: string[]): Promise<any | null> {
    ...
    const renterDocs = await Promise.all(
        [targetRenterId, ...connectedRenterIds].map(id => adminDB.collection('renters').doc(id).get())
    );

    for (const doc of renterDocs) {
        if (!doc.exists) continue;
        const data = doc.data()!;
        clusterMembers.push({ id: doc.id, trustScore: data.trustScore, name: data.name });
...
export async function POST(req: Request) {
    try {
        ...
        if (connectedRenterIds.length === 0) {
            return NextResponse.json({ message: "No connected renters found. No cluster to analyze." });
        }

        // 3. Analyze the cluster for fraudulent signals
        const fraudAnalysis = await analyzeClusterForFraud(renterId, connectedRenterIds);
