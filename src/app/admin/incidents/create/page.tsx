...
export default function AddIncidentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const evidenceUrls = await uploadEvidenceFiles(`incidents/${renterId}`, evidenceFiles);
    ...
    <div className="max-w-2xl mx-auto py-10 px-4">
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg p-8 space-y-6">
        ...
        <div>
          ...
          <input id="renterId" type="text" value={renterId} onChange={(e) => setRenterId(e.target.value)} placeholder="Enter Renter's User ID" required />
          <p>For this demo, manually enter the user ID. In a real app, this would be a user search feature.</p>
        </div>
        ...
        <div>
          ...
          <textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} rows={4} placeholder="Provide a detailed description..." required />
        </div>
        ...
        <div className="flex items-center">
          <input id="notifyRenter" type="checkbox" checked={notifyRenter} onChange={(e) => setNotifyRenter(e.target.checked)} />
          <label htmlFor="notifyRenter" className="ml-2">Notify Renter via Email</label>
        </div>
