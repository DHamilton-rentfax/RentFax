  ...
  const addOnGroups = [
    {
      ...
      description: "Enhance your screening accuracy and detect fraud patterns.",
      items: [
        {
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="text-center py-20 bg-gradient-to-b from-background to-muted/30">
        ...
        <p className="text-lg text-muted-foreground">
          Choose the plan that fits your business — and customize it with
          add-ons.
        </p>
      </section>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {group.items.map((addon) => {
                const selected = selectedAddOns.some(
                  (a) => a.priceId === addon.priceId,
                );
                return (
                  <div
                    key={addon.name}
                    className={`p-4 border rounded-xl transition-colors ${selected ? "border-primary bg-primary/5" : "border-muted"}`}
                  >
                    <div className="font-medium mb-2">{addon.name}</div>
                    <div className="flex justify-between items-center">
                      <Link
                        href={`/docs/addons/${addon.slug}`}
                      ...
                      <button
                        ...
                        className={`px-3 py-1 text-sm rounded-md ${selected ? "bg-red-500/10 text-red-400" : "bg-primary/10 text-primary"}`}
                      >
                        {selected ? "Remove" : "Add"}
                      </button>
                    </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-background p-6 rounded-xl shadow-lg w-full max-w-md">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Enter tenant details to generate a RentFAX Risk Report.
            </p>
      ...
      {showContactModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-background p-6 rounded-xl shadow-lg w-full max-w-md">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Tell us about your organization — our team will reach out.
            </p>
            <form className="space-y-3">
