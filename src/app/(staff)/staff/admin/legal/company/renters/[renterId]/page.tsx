...
export default function RenterProfile({ params }: { params: { renterId: string } }) {
  ...
  useEffect(() => {
    async function load() {
      const snap = await getDoc(doc(db, "renters", params.renterId));
      if (snap.exists()) {
        setRenter({ id: snap.id, ...snap.data() });
        const res = await fetch(`/api/fraud?id=${params.renterId}`);
  ...
  return (
    <div className='p-6 grid grid-cols-1 md:grid-cols-3 gap-6'>
      <div className='md:col-span-1'>
        <FraudScoreCard renterId={params.renterId} userId={user.uid} />
      </div>
