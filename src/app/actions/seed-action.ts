...
export async function seedInitialUsers() {
    ...
    try {
        await batch.commit();
        return { success: true, message: `${mockUsers.length} users have been seeded successfully.` };
    } catch (error: any) {        
...
export async function runFraudAnalysisOnSeededUsers() {
    try {
        ...
        return { success: true, message: `Fraud analysis completed for ${processedCount} users.` };
