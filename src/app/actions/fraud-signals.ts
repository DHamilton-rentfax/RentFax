...
// Calculates and stores the fraud summary for a renter
async function updateFraudSummary(renterId: string, signals: any[]) {
  if (!renterId) return null;

  // Calculate score (0-100) based on the highest confidence signal
  const score = Math.round(
    signals.reduce((max, s) => Math.max(max, s.confidence), 0) * 100,
  );

  // Determine if an alert should be triggered
  const alert = score >= 75 || signals.length >= 3;
...
export async function getFraudSignals(uid: string) {
  ...
  if (duplicateIPs.length > 0) {
    ...
    if (userData.createdAt && userData.createdAt.toDate) {
      ...
      const recentAccounts = duplicateIPs.filter((user: any) => {
        if (user.createdAt && user.createdAt.toDate) {
          const otherUserCreationTime = user.createdAt.toDate().getTime();
          return (
            Math.abs(currentUserCreationTime - otherUserCreationTime) <
            twentyFourHours
          );
        }
        return false;
  ...
  // Calculate and store the fraud summary
  const summary = await updateFraudSummary(uid, signals);
