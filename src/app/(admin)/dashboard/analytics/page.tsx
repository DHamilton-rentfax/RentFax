    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        rentersSnap.forEach((doc) => {
          ...
          const bucket =
            score >= 75 ? "Safe" : score >= 50 ? "Moderate" : "Risky";
          riskDist[bucket]++;
        });
        ...
        const newStats: Stats = {
          ...
          incidentTypes: Object.entries(incidentTypes).map(([name, value]) => ({
            name,
            value,
          })),
          riskBuckets: Object.entries(riskDist).map(([name, value]) => ({
            name,
            value,
          })),
          disputesOpen: disputesSnap.docs.filter(
            (d) => d.data().status === "open",
          ).length,
          disputesResolved: disputesSnap.docs.filter(
        ...
        console.error(err);
        setError(
          "Could not load analytics data. Check permissions and Firestore indexes.",
        );
      } finally {
        setLoading(false);
      }
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Breakdown of renter risk scores.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ChartContainer
